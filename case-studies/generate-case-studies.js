...
const CASES = [
  {
    ...
    decisions: 'Client side only, no backend, was chosen over a server side calculator because subnetting math has no need to touch a network at all: doing it in the browser removes a whole class of availability and latency concerns for a tool that should feel instant. Permalinks were encoded directly into the URL query string rather than stored server side, trading a slightly longer URL for zero database, zero expiry, and zero privacy exposure of what someone calculated.',
  },
  {
    slug: 'passcheck',
    name: 'passcheck.',
    ...
    decisions: 'The k-anonymity design (send only a 5 character hash prefix, match the rest locally) was chosen over the simpler option of hashing and sending the full SHA-1, because a full hash is still enough for the API operator to correlate requests to a specific password over time. The extra client side matching logic is a deliberate tradeoff of a little more code for a real, not just claimed, privacy guarantee.',
  },
  {
    slug: 'inspect',
    name: 'inspect.',
    ...
    decisions: 'A small Express backend was necessary here, unlike the fully client side tools in this portfolio, because DNS lookups (SPF, DKIM, DMARC) and header fetches against an arbitrary third party domain are blocked by browser CORS policy and would leak the visitor doing the lookup. That backend dependency is exactly why it was the one tool that went silently down: it is the tradeoff of needing a server at all, and the fix was adding it to the site wide uptime checker that now polls every 5 minutes and drives the status dot on every project card.',
  },
  {
    slug: 'cert-check',
    name: 'cert-check.',
    ...
    decisions: 'A direct TLS socket connection was used to read the live certificate chain rather than relying on a third party certificate transparency log API, so the result always reflects exactly what a real client connecting right now would see, including misconfigurations like an incomplete chain that a log lookup alone would not surface. That is a deliberate accuracy over convenience tradeoff.',
  },
  {
    slug: 'acl-gen',
    name: 'acl-gen.',
    ...
    decisions: 'A single internal rule model was designed first, before any vendor renderer was written, specifically so adding a sixth vendor format later only means writing one new renderer against an existing model rather than reworking the whole tool. Shadow and conflict detection runs against that same internal model rather than against each rendered syntax separately, so the logic is written and tested once instead of five times.',
  },
  {
    slug: 'cv-builder',
    name: 'CV Builder.',
    ...
    approach: 'Built with Next.js and Supabase for auth and storage, with DeepSeek powering the AI features: a real time ATS score sidebar, tailoring suggestions against a pasted job description, a skill gap analyzer, salary insights, an AI interview coach, and a LinkedIn optimizer, on top of 14 templates and PDF import and export. It later moved to share the timfas.com domain at the /cv-builder/ path alongside the rest of the portfolio.',
    ...
    decisions: 'Supabase was chosen over a self managed database specifically to keep auth, storage, and Postgres in one managed service for a solo built product, trading some vendor lock-in for a much smaller operational surface. DeepSeek was chosen for the AI features over a larger frontier model provider mainly on cost per request at the volume a free tier tool needs to sustain, with the interview coach and tailoring prompts written to be portable to another provider if that tradeoff changes later.',
  },
];
